...
if($news){
    ...
    <div class="videoNewsSlider">
        ...
        <div>
            <a <?php echo !$news['images']?'class="imgWrap no-image"':'class="imgWrap"'?> href="<?php echo Yii::$app->urlManager->createAbsoluteUrl('/video-news/'.$news['translit'])?>" title="<?php echo $news['t']['title']?>" >
                <?php
                if($news['images']){
                    $path = ImageHelper::buildImgPathThumbnail(current($news['images']));
                }else{
                    if($news['video_id']){
                        $path = 'http://img.youtube.com/vi/'.$news['video_id'].'/0.jpg';
                    }else{
            ...
            <p class="videoCaption">
                <a href="<?php echo Yii::$app->urlManager->createAbsoluteUrl('/video-news/'.$news['translit'])?>"
                   title="<?php echo $news['t']['title']?>"><?php echo $news['t']['title']?></a>
            </p>
            <p class="videoDesc">
